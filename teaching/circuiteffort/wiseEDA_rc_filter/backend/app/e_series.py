...
E_SERIES = {
    ...
    "E96": [1.00,1.02,1.05,1.07,1.10,1.13,1.15,1.18,1.21,1.24,1.27,1.30,1.33,1.37,1.40,1.43,1.47,1.50,1.54,1.58,1.62,1.65,1.69,1.74,
...
def nearest_series(value: float, series: str, vmin: float, vmax: float) -> float:
    value = min(max(value, vmin), vmax)
    decade = 10 ** math.floor(math.log10(value))
    norm = value / decade
    base = E_SERIES[series]
    nearest = min(base, key=lambda x: abs(x - norm))
    cand = nearest * decade
    # clamp
    while cand < vmin:
        cand *= 10
